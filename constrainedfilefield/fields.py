...
class ConstrainedFilePathField(models.FilePathField):
    """
    A FilePathField with additional constraints. Namely, the file size and type can be restricted. If
    using the types, the magic library is required. Setting neither a file size nor type behaves
    like a regular FileField.
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ConstrainedFilePathField, self).__init__(*args, **kwargs)

    def clean(self, *args, **kwargs):
        data = super(ConstrainedFilePathField, self).clean(*args, **kwargs)
    ...
    def formfield(self, **kwargs):
        ...
        """
        formfield = super(ConstrainedFilePathField, self).formfield(**kwargs)
        if self.js_checker:
            formfield.widget.attrs.update(
                {'onchange': 'validateFileSize(this, %d);' % (self.max_upload_size,)})
        return formfield
    ...
    def deconstruct(self):
        name, path, args, kwargs = super(ConstrainedFilePathField, self).deconstruct()
        if self.max_upload_size:
            kwargs["max_upload_size"] = self.max_upload_size
        if self.content_types:
            kwargs["content_types"] = self.content_types
